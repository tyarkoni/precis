...
class Item:
    ...
    def __init__(self, text, id=None, offset=0, reverse=False):
        self.text = text
        self.id = id
        self.offset = offset
...
#     m = Measure('items3.txt', 'scales3.txt', missing='delete')
#     a = Abbreviator(pop_size=200, item_cost=0.05, max_items=5, min_r=0.2)
#     # import cProfile
#     # cProfile.run('abb = a.abbreviate(m, n_gens=100)')
#     abb = a.abbreviate(m, n_gens=200)
    
#     f = open('measure_info.txt', 'w')
#     f.write(str(abb))
#     f.close()

#     with open('simple_key_maxitems=5_itemcost=0.05.txt', 'w') as f:
#         f.write('item\t%s\n' % '\t'.join(m.y_names))
#         row_nums = np.atleast_2d(np.arange(1, abb.key.shape[0]+1)).T
#         np.savetxt(f, np.hstack((row_nums, abb.key)), delimiter='\t', fmt='%d')
#     a.make_figure(panels=['corr-observed', 'corr-cross'])
    # a.plot_generational_stats()
