...
def composite(generator, panels, measure=None, rows=1, cols=None, **kwargs):
    ...
    Args:
        generator (Generator): the Generator instance containing the data to
            extract.
        panels (list): A list of strings specifying the figure panels. Can be:
        ...
        measure (Measure): Optional Measure instance to pass on to other
            plotting functions.
        rows (int): Number of rows in figure. Defaults to single row.
        cols (int): Optional number of columns. If None, will determine cols
            based on the number of panels and rows.
    ...
    for i, p in enumerate(panels):
        ...
        if p.startswith('hist'):
            ...
            scale_correlation_matrix(
                measure, corr_with=p.split('-')[-1], ax=ax)

    adjust_figure(fig, **kwargs)


def scale_scatter_plot(measure, rows=1, cols=None, jitter=0.0, alpha=0.3,
                       trend=False, text=True, totals=False, **kwargs):
    ''' Generate scatterplot of abbreviated vs. original scores for each scale.

    Args:
        measure (Measure): A Measure instance
        rows (int): Number of rows in scale grid
        cols (int): Number of columns in scale grid. If None, will set to 
            n_scales/rows
        jitter (float): Amount to jitter x and y values by (uniform
            distribution in range [-jitter, jitter]). This is useful when there
            are too many data points to see clear trends.
        alpha (float): Alpha level (opacity) of data points.
        trend (bool): Whether or not to plot the regression line.
        text (bool): Whether or not to plot the r-squared text label.
        totals (bool): If true, adds a subplot for the total score across all
            scales.
    Returns:
    ...
    if totals:
        names.append('Total')
        abbreviated_y = np.hstack(
            (abbreviated_y, np.atleast_2d(np.sum(abbreviated_y, 1)).T))
        original_y = np.hstack(
            (original_y, np.atleast_2d(np.sum(original_y, 1)).T))
        r_squared.append(
            np.corrcoef(abbreviated_y[:, -1], original_y[:, -1])[0, 1] ** 2)
    ...
    for i in range(abbreviated_y.shape[1]):
        ...
        if text:
            ...
            plt.text(0.88, 0.08, r'R$^2$ = %.2f' % r_sq,
    ...
    plt.subplots_adjust(
        left=0.07, right=0.95, top=0.95, bottom=0.07, hspace=0.4, wspace=0.3)
    adjust_figure(**kwargs)


def scale_correlation_matrix(measure, corr_with='cross', text=True, ax=None,
                             **kwargs):
    ''' Plot the correlation matrix between scales. 
    Args:
        corr_with (str): Which sets of variables to correlate.
            'original': Plots intercorrelations between original (i.e., real) 
                scores
            'predicted': Plots intercorrelations between predicted scores.
            'cross': Plots correlations between original and predicted scores.
        text (bool): Boolean indicating whether to display the actual
            correlation values on the matrix.
    ...
    if corr_with == 'cross':
        data = np.corrcoef(measure.predicted_y.T, measure.dataset.y.T)[
            0:measure.n_y, measure.n_y::]
...
def history(generator, **kwargs):
    ''' Plot evolution of best measure across generations: total cost,
    r-squared, number of items.

    Args:
        generator (Generator): the generator to plot history for
        kwargs: optional keyword args to pass to adjust_figure
    Returns:
        The current Figure instance.
    '''
